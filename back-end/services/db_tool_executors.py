...
class AIToolExecutor:
    ...
    def _fetch_tables_with_config(
        ...
        with get_tool_connection(db_config) as conn:
            ...
            schema = db_config.get("schema", "public")
            query, params = adapter.get_all_tables_for_cache(
                effective_db_name, schema=schema
            )
            cursor.execute(query, params) if params else cursor.execute(query)
    ...
    def _fetch_tables_and_columns(
        ...
        columns = AIToolExecutor._batch_fetch_columns(
            db_config, tables, db_type, db_name
        )
    ...
    def _batch_fetch_columns(
        db_config: dict, tables: List[str], db_type: str, db_name: str = None
    ) -> Dict[str, List]:
        """
        ...
        with get_tool_connection(db_config) as conn:
            ...
            query, params = adapter.get_batch_columns_for_tables(
                effective_db_name, tables, schema=schema
            )
    ...
    @staticmethod
    def _execute_query_with_db_config(
        db_config: Dict, connection: Dict, query: str, max_rows: int
    ) -> Dict:
    ...
    def _get_table_indexes(
        ...
        try:
            adapter = get_adapter(db_type)
            query, params = adapter.get_indexes_query(
                table_name, db_name=database, schema=schema
            )
    ...
    def _get_foreign_keys(
        ...
        try:
            adapter = get_adapter(db_type)
            query, params = adapter.get_foreign_keys_query(
                table_name, db_name=database, schema=schema
            )
